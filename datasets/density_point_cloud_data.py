...
class FrustumDensityDataset(BaseDensityDataset):
...
class BirdsEyeViewDataset(BaseDensityDataset):
    ...
    def get_bev_from_beams(self, feats):
        n_beams = feats.shape[0]
        norm_shift = NormalizeShift()
        swap = NegZForward2YForward()

        imgs = np.zeros([n_beams, self.bev_dims[0], self.bev_dims[1]])

        for i in range(n_beams):
            img = np.zeros(self.bev_dims)
            points = feats[i]
            points = norm_shift.fit_transform(points)
            points = swap(points).data.numpy()

            mins = points.min(axis=0)
            maxs = points.max(axis=0)

            bev = point_cloud_2_birdseye(
                points,
                res=self.bev_res,
                side_range=(mins[0], maxs[0]),
                fwd_range=(mins[1], maxs[1]),
                height_range=(mins[2], maxs[2])
            )
            #plt.imshow(bev, cmap="nipy_spectral", vmin=0, vmax=255)
            #plt.show()
            h, w = bev.shape

            h_max = min(h, self.bev_dims[0])
            w_max = min(w, self.bev_dims[1])

            img[:h_max, :w_max] = bev[:h_max, :w_max]
            imgs[i] = img
    ...
    def get_wandb_plot(self, wandb_run, batch, y, y_hat=None, eval_dict=None):
        ...
        # assuming point clouds are stacked (fixed size)
        # see_keys_to_stack = set(["num_targets", "points"]) in datasets.collate_fn.collate_density_reg()
        points_per_batch = batch["verts"].shape[0] // batch_size
        ...
        for i, n in enumerate(num_targets):
            ...
            bev = batch["bev"][idx:(n+idx)]

            # flip 3-D tensor manually
            feats = swap(batch["points"][idx:(n+idx), :, :3])
            feats[:, :, 0] = -1*feats[:, :, 0]

            feats_rgb = self.get_pc_feats(feats)

            gt_labs = np.round(y[idx:(n+idx), :].data.numpy().flatten().tolist(), 1)

            if y_hat is not None:
                pred_labs = np.round(y_hat[idx:(n+idx)].data.numpy().flatten().tolist(), 1)

                labels = [f"gt: {gt_labs[j]}\npred: {pred_labs[j]}" for j in range(len(gt_labs))]
            else:
                labels = [f"gt: {gt_labs[j]}" for j in range(len(gt_labs))]
            ...
            # log bev images
            bev = bev.unsqueeze(1) * 255 # unnormalize
            caption = [str(int(gt)) for gt in gt_labs]
            caption = "{" + ",".join(caption) + "}"
            bev_dict = wandb_run.get_img_log(
                bev,
                "Bird's Eye View",
                caption=caption
            )
            dict_list.append(bev_dict)
    ...
    def __getitem__(self, idx):
        ...
        verts, sem_seg = self.shuffle_points(verts, sem_seg)
        if self.sample_points:
            verts, batch["normals"], sem_seg = self.sample_point_cloud_tensor(
                verts,
                batch["normals"],
                sem_seg,
                self.sample_points)
        sem_seg = self.binarize_seg_scores(sem_seg, tol=0.5)

        verts = self.get_points_by_seg(verts, sem_seg)

        #sem_seg = self.get_points_by_seg(sem_seg, sem_seg) # filter to match vert shape
        ...
        bev = self.get_bev_from_beams(feats)
        bev /= 255.0 # normalize
        ...
        """if sem_seg is not None:
            x.update({
                "ss_scores": sem_seg[:self.wandb_point_limit, :]  # truncate (shuffled) points for wandb viz
            })"""
