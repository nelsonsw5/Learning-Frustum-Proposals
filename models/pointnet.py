...
class PointNet4DFeatureExtractor(nn.Module):
    """
    PointNet model that requires 4D input:
        - (batch size, # sub clouds, # points, input channels)
    args:
        in_channels (int): Number of channels in the input pointcloud
            (default: 3, for X, Y, Z coordinates respectively).
        feat_size (int): Size of the global feature vector
    ...
    def __init__(self,
        ...
        super(PointNet4DFeatureExtractor, self).__init__()
    ...
    def forward(self, x: torch.Tensor):
        ...
        # Number of points
        bs, subclouds, in_channels, num_points = x.shape

        x = x.view(-1, in_channels, num_points)
        ...
        # reshape to 4D
        x = x.view(bs, subclouds, self.feat_size, num_points)
